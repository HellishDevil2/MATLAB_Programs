...
%BandStop Magnitude Charecterstics
...
ylabel("20Log(|S11|)");
title("Band Stop S11 Amplitude");

subplot(2,2,2);
plot(FREQ1_GHZ,LOGMAG2);
xlabel("Frequency (GHz)");
ylabel("20Log(|S12|)");
pass_band = find(LOGMAG2-min(LOGMAG2)<3)';
disp("Lower and Upper cutoff frequencies are ");
disp(FREQ1_GHZ(pass_band));
title("Band Stop S12 Amplitude");

subplot(2,2,3);
plot(FREQ1_GHZ,LOGMAG3);
xlabel("Frequency (GHz)");
ylabel("20Log(|S21|)");
title("Band Stop S21 Amplitude");

subplot(2,2,4);
plot(FREQ1_GHZ,LOGMAG4);
xlabel("Frequency (GHz)");
ylabel("20Log(|22|)");
title("Band Stop S22 Amplitude");
